...
public class LinkedTransferQueue<E> extends AbstractQueue<E>
    ...
    // CAS methods for fields ，用val去替换tail，原始tai为cmp。
    ...
    private E xfer(E e, boolean haveData, int how, long nanos) {
        if (haveData && (e == null))      //如果有data，但是e又为null。则报错。
        ...
        for (;;) {                            // restart on append race  自旋。
            ...
            for (Node h = head, p = h; p != null;) { // find & match first node     //从头开始找。
                ...
                Object item = p.item;
                if (item != p && (item != null) == isData) { // unmatched     都有值
                    if (isData == haveData)   // can't match   //不能匹配。
                        break;
                    if (p.casItem(item, e)) { // match   把p的item设为e。
                        ...
                        LockSupport.unpark(p.waiter);     //唤醒p。
            ...
            if (how != NOW) {                 // No matches available   //当前位置没有匹配到的。
                if (s == null)
            ...
            if (p == null && (p = head) == null) {          //当队列中没有任何节点时候。
                ...
                if (p != t) {                 // update if slack now >= 2
                    while ((tail != t || !casTail(t, s)) &&               //t为tail时候
                           ...
                           (s = s.next) != null && s != t);                      //tail的后面的后面也不为null。
                }
                return p;
            }
        }
     ...
     * 
     * 返回第一个没有match的节点，以给定模式。
     * 
     * 主要用于方法，isEmpty, hasWaitingConsumer.
     */
    private Node firstOfMode(boolean isData) {
        for (Node p = head; p != null; p = succ(p)) {
            if (!p.isMatched())            //如果p没有match。
     ...
     * 
     * 
     * 被Spliterator用的firstOfMode版本。
     * 
     * 需要检查为null。以及自己链接自己，即只有一个节点时候。
     */
     ...
     * 
     * 被size和getWaitingConsumerCount使用。
     * 
     * 
     * 获取某一种模式的个数。
     */
    private int countOfMode(boolean data) {
        int count = 0;
        for (Node p = head; p != null; ) {
            if (!p.isMatched()) {             //没有匹配上
                if (p.isData != data)
                    return 0;
                if (++count == Integer.MAX_VALUE) // saturated   已经最大了。
                    break;
            }
            Node n = p.next;
            if (n != p)
