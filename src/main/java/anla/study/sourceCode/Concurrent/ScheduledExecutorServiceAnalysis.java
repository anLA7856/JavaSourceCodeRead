 ...
 *   }
 * }}</pre>
 *这里有一个类的方法，设置一个ScheduledExecutorService，每10秒钟发出一个小时的嘟嘟声：
 * @since 1.5
 * @author Doug Lea
 */
public interface ScheduledExecutorService extends ExecutorService {
     ...
     * 
     * 创建并执行在给定的初始延迟之后首先变为启用的周期性动作，
     * 并且随后在给定的周期内; 那就是在initialDelay那initialDelay+period之后 
     * 就会开始执行 initialDelay + 2 * period，等等。
     */
     ...
     * 
     * 创建并执行一个定期动作，在给定的初始延迟之后首先启用，然后在一个执行的终止
     * 和下一个执行的开始之间给定延迟。
